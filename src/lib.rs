...
pub struct SecretKey {
    ...
    ml_kem_secret: [u8; ML_KEM_768_SECRET_KEY_BYTES_LENGTH],
...
impl XWing {
    ...
    fn derive_keypair_from_seed(seed: [u8; 96]) -> Result<(SecretKey, PublicKey), Error> {
        let deterministic_d: [u8; 32] = seed[0..32].try_into().expect("seed is 96 bytes");
        let deterministic_z: [u8; 32] = seed[32..64].try_into().expect("seed is 96 bytes");
        ...
        let secret = SecretKey {
            ml_kem_secret: ml_kem_secret.as_bytes().into(),
            x25519_secret,
            x25519_public,
        };

        let public = PublicKey {
            ml_kem_public: ml_kem_public.as_bytes().into(),
            x25519_public,
        };
    ...
    #[cfg(test)]
    pub fn encapsulate_deterministic<Pk: AsMut<PublicKey>>(
        seed: [u8; 64],
        mut public_key: Pk,
    ) -> Result<(SharedSecret, Ciphertext), Error> {
...
mod tests {
    ...
    fn encapsulator_and_decapsulator() {
        let csprng = OsRng;
        let (decapsulator, decapsulator_public_key) = XWingDecapsulator::new(csprng)
            .expect("XWingDecapsulator should generate keys successfully");
        let encapsulator = XWingEncapsulator::new(decapsulator_public_key, csprng);

        let (encapsulator_shared_secret, encapsulator_cipher) = encapsulator
            .encapsulate()
            .expect("honest encapsulation works");
    ...
    fn test_vectors_encaps(
        ...
        let (_encaps_sk, encaps_ct) =
            XWing::encapsulate_deterministic(eseed, public_key).expect("decapsulation works");
    ...
    fn test_vector_1_ietf() {
        ...
        const ESEED: [u8; 64] = hex!(
            "badfd6dfaac359a5efbb7bcc4b59d538df9a04302e10c8bc1cbf1a0b3a5120ea17cda7cfad765f5623474d368ccca8af0007cd9f5e4c849f167a580b14aabdef"
        );
    ...
    fn test_vector_3_ietf() {
        ...
        const ESEED: [u8; 64] = hex!(
            "e770d01efde86e721a3f7c6cce275dabe6e2143f1af18da7efddc4c7b70b5e345db93cc936bea323491ccb38a388f546a9ff00dd4e1300b9b2153d2041d205b4"
        );
